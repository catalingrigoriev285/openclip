...
- `updater.rs`: update UI on top of `crate::update` — `start_update_check` (spawned from `App::new` when `check_updates`, and by the About / General "Check for updates" button), `poll_update` (called at the top of `ui()`), the `Update to vX` chip in the status strip's Idle arm, and `update_dialog` (release notes, Download and install → progress bar → Restart now; falls back to "Open release page"), shown last in the full layout only.
